...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // DEBUG: log full Supabase sign-in response for diagnostics
    if (signErr) {
      console.error('Supabase signIn error details:', {
        message: signErr.message,
        status: signErr.status ?? 'no-status',
        hint: signErr.hint ?? null,
        details: signErr
      });
    }
    console.error('Supabase signIn result for', email, { signData, signErr });

    if (signErr || !signData) {
      return res.status(signErr?.status || 401).json({ message: signErr?.message || 'Invalid email or password.' });
    }
...
app.post('/api/auth/reset-password', async (req, res) => {
  try {
    ...
    // so use the REST endpoint as a lightweight workaround.
    // Note: This uses the public anon key pattern and the /auth/v1/ endpoint which is intended for client-side flows,
    // but for server-side resetting with a token you can call the REST endpoint as below.
    const fetchUrl = `${process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL}/auth/v1/user`;
    try {
      const resp = await fetch(fetchUrl, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_SERVICE_ROLE_KEY}`
        },
        body: JSON.stringify({ password, token })
      });
      if (!resp.ok) {
        const errText = await resp.text();
        console.error('Reset password REST error', resp.status, errText);
